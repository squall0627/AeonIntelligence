...
def default_translate_llm() -> LLMEndpoint:
    try:
        llm = LLMEndpoint.from_config(
            LLMEndpointConfig(supplier=DefaultModelSuppliers.ALIBABA, model=LLMName.qwen_32b)
        )
        return llm
...
class Translator:
    def __init__(self,
        ...
        self.source_language = source_language.value if isinstance(source_language, Language) else source_language
